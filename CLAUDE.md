...
- **`e2e-dev/`** (config `playwright.dev.config.ts`, run via `npm run test:e2e:dev`) — **authenticated flows** (Quick Start anonymous auth, adding expenses) driven against the real **dev** project (`easy-split-dev-1cfa3`). These write throwaway anonymous users + groups and clean up after themselves via the app's Delete Account flow (`cleanupCurrentAccount` in `e2e-dev/helpers.ts`, best-effort in `afterEach`). Runs serially. Two modes: **local** boots the dev server on port 5174 loading `.env.local`; **remote** targets an already-deployed URL when `E2E_BASE_URL` is set (no server started). In CI this runs in the **`e2e-dev` job after `deploy-test`**, pointed at `https://preview.slice.pages.dev` — i.e. it tests the just-deployed dev site (push to `main` only, not PRs).
